...
Purpose:
    Defines, trains, and saves the GRU-based next-word prediction model.
    Mirrors model_lstm.py exactly in structure and hyperparameters, with
    the LSTM layer replaced by a GRU layer -- this keeps the LSTM vs GRU
    comparison fair, since only the recurrent layer type changes.
...
# ---------- Constants ----------
# Kept identical to model_lstm.py (except GRU_UNITS naming) so the
# LSTM vs GRU comparison isolates the effect of the layer type only.
...
def build_gru_model(vocab_size: int, input_length: int) -> Sequential:
    """
    Build and compile the GRU next-word prediction model.

    Architecture:
        Input -> Embedding -> GRU -> Dense (softmax over vocabulary)
...
def train_gru_model() -> Tuple[Sequential, int]:
    """
    Run the full pipeline: load data, preprocess it, build the GRU model,
    ...
    model = build_gru_model(vocab_size=vocab_size, input_length=input_length)
    model.summary()
    ...
    # Step 6: Save the tokenizer as well (identical to the one produced by
    # model_lstm.py, since both use the same corpus and preprocessing logic)
    save_tokenizer(tokenizer, TOKENIZER_SAVE_PATH)

    print(f"\nGRU model saved to: {MODEL_SAVE_PATH}")
